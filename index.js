class CoreXStream{
    ...
    VideoStream(VideoFolder, TagName, req, res ){
        ...
        if (req.query[TagName].match(/^[a-z0-9-_ ]+\.(pm4|mov)$/i)){
            if(this._fs.existsSync(filePath)){
                ...
                // this._fs.stat(filePath, (err, stat) => {
                //     if (err) {
                ...
                //         fileStream.pipe(res);
                //     }
                //     const Ticend = Date.now()
                //     console.log(Ticend - Ticstart)
                // });
            ...
            res.status(404).send(`File name not match with reg expression: ${req.query[TagName]}`)
        }
    }
} 
